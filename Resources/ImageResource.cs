...
{
    /// <summary>
    /// Represents a single image in ImGui.Window.
    /// </summary>
    ...
    public class ImageResource
    {
        ...
        private ImageResource(Application application, Bitmap image)
        {
            _img = image;
        ...
        public static ImageResource FromFile(Application application, string path)
        {
            return FromBitmap(application, (Bitmap)Image.FromFile(path));
        }
        ...
        /// <remarks>To load built-in images, see <see cref="ImageResources"/>.</remarks>
        public static ImageResource FromResource(Application application, Assembly assembly, string resourceName)
        {
            return FromStream(application, assembly.GetManifestResourceStream(resourceName)!);
        }
        ...
        /// <remarks>To load built-in images, see <see cref="ImageResources"/>.</remarks>
        public static ImageResource FromStream(Application application, Stream stream)
        {
            return FromBitmap(application, (Bitmap)Image.FromStream(stream));
        }
        ...
        /// <remarks>To load built-in images, see <see cref="ImageResources"/>.</remarks>
        public static ImageResource FromBitmap(Application application, Bitmap image)
        {
            return new ImageResource(application, image);
        }
        ...
        {
            if (_ptr != IntPtr.Zero)
                _application.ImageFactory.UnloadImage(_ptr);
        ...
        private IntPtr GetPointer()
        {
            ...
            return _ptr = _application?.ImageFactory.LoadImage(_img) ?? IntPtr.Zero;
        }
    }
}
